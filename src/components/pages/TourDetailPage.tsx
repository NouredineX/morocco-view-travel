...
export default function TourDetailPage({ locale, slug }: { locale: Locale; slug: string }) {
  ...
  const whatsappUrl = `https://wa.me/212708228026?text=${encodeURIComponent(`Hello Travelling Through Morocco, I would like to book the "${title}" (${tour.duration} days).`)}`;
  ...
  const tourSchema = {
    ...
    "itinerary": itineraryDays.map((day) => ({
      "@type": "HowToStep",
      "name": getTranslated(day, 'title', locale),
      "text": getTranslated(day, 'content', locale)
    }))
  };

  return (
    <div id="tour-detail-page">
      {/* Tour Detail JSON-LD Schema */}
      <script
      ...
      <section className="page-hero" style={{ background: `linear-gradient(rgba(10, 15, 26, 0.75), rgba(10, 15, 26, 0.9)), url("/images/${tour.image}") no-repeat center center/cover`, paddingTop: '10rem', paddingBottom: '5rem', textAlign: 'center' }}>
        <div className="page-hero-content" style={{ maxWidth: '800px', margin: '0 auto', padding: '0 1rem' }}>
          ...
          <div className="breadcrumb" style={{ display: 'flex', justifyContent: 'center', gap: '0.5rem', color: 'var(--text-secondary)', marginTop: '1rem' }}>
            ...
            <Link href={getLocalizedPath('tours', locale)} style={{ color: 'inherit' }}>Tours</Link>
            <span className="separator">/</span>
